...
const { state, saveCreds } = await useMultiFileAuthState('./src/sesi')
    ...
    tio.sendMessage(callerId, { text: `*Sistem otomatis block!*\n*Jangan menelpon bot woy, gila lu ya kek bocil najis, kerja sana ganggu aja lu pengangguran*\n*Silahkan Hubungi Owner Untuk Dibuka! bye dek.*`}, { quoted : messa })
    await sleep(8000)
    ...
    tio.ev.on('groups.update', async tioxd => {
       ...
       tio.send5ButImg(tioxd[0].id, `「 *Group Settings Change* 」\n\nGroup telah ditutup oleh admin, Sekarang hanya admin yang dapat mengirim pesan, yahaha hayyuk !`, `GROUP MESSAGE`, wm_tiodev, [])
       } else if(tioxd[0].announce == false) {
       tio.send5ButImg(tioxd[0].id, `「 *Group Settings Change* 」\n\nGroup telah dibuka oleh admin, Sekarang peserta dapat mengirim pesan yahaha yahuuk !`, `Group Settings Change Message`, wm_tiodev, [])
...
tekswell = `Halo hama, @${num.split('@')[0]} 🌀\n*Selamat Datang Di Grup Sampah ini semoga cepet keluar*${metadata.subject}*\n\n🥂 Name : @${num.split('@')[0]}\n🤔 Nomer : ${num.split('@')[0]}\n😭 Group : ${metadata.subject}\n🔥 Member : ${metadata.participants.length}`
...
teksbye = `Selamat Tinggal keberadaan mu tidak di harapkan disini bye ya tod @${num.split("@")[0]} 😠\n\n🐽 Name : @${num.split('@')[0]}\n🐕️ Nomer : ${num.split('@')[0]}\n🥦 Group : ${metadata.subject}\n🥦 Member : ${metadata.participants.length}`
...
tekspromote = `Selamat @${num.split("@")[0]} Atas Kenaikan Jabatannya Di Grup wkwk ${metadata.subject}`
...
if (update.connection == "open" || update.receivedPendingNotifications == "true") {
	...
	lolcatjs.fromString(`✅ Berhasil gan Tersambung Ke ` + JSON.stringify(tio.user, null, 2))
	global.creator = ['6289637196946']
	...
	let txtcrea = `Script di pake oleh\nID: ${global.owner}@s.whatsapp.net`
